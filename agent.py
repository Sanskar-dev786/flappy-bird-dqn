...
#Dictionary for shaving run info
RUNS_DIR = "runs"
os.makedirs(RUNS_DIR, exist_ok=True)

# 'Agg': used to generate plots as images and save them to disk without displaying them on the screen. This is useful for running code on servers or in environments without a graphical interface.
matplotlib.use('Agg')
...
class Agent: 
    ...
    #  env = gym.make("FlappyBird-v0", render_mode="human", use_lidar=True)
        ...
        # Create policy and target networks. Number of nodes in the hidden layer can be adjusted.
        policy_dqn = DQN(num_states, num_actions, self.fc1_nodes, self.enable_double_dqn).to(device)

        if is_training:
            # Initialize replay memory, epsilon, target network
            memory = ReplayMemory(self.replay_memory_size)
            epsilon = self.epsilon_init

            # Create target network and copy weights from policy network
            target_dqn = DQN(num_states, num_actions, self.fc1_nodes, self.enable_double_dqn).to(device)
            target_dqn.load_state_dict(policy_dqn.state_dict())


            # Policy network optimizer "Adam" optimizer can be swapped to someting else.
            self.optimizer = torch.optim.Adam(policy_dqn.parameters(), lr= self.learning_rate_a)

            # List to keep track of epsilon decay
            epsilon_history = []

            # Track the number of steps taken to determine when to update the target network
            step_count = 0
        ...
        # Train INFINITELY , manully stop the run when you are satisfied (or unsatisfied) with the results. 
        for episode in itertools.count():
            ...
            # Keep track of the rewaards collected per episode.
